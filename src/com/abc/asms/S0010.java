...
//		AuthCheck.checkAuthandTransition(request, response, "/JSP/S0010.jsp",
//				puf.getPermitList("accounts"));
...
//				response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (saleservice.parse(request).getTrade_name().isEmpty()) {
		...
		//			if (!(checklength.checkLength(String.valueOf(saleservice.parse(request).getUnit_price()), 10))) {
		//				errMsg.add("単価が長すぎます。");
		//			}
		//		}
		...
		//						&& !(saleservice.parse(request).getUnit_price() == 0)) {
